...
import PartnerListModalContent from '../modal/modalContent/PartnerListModalContent';
...
const AdminOrdersTable = () => {
  ...
  const [isModalPartnerOpen, setIsModalPartnerOpen] = useState<boolean>(false);
  ...
  return (
    <AO.TableFilterWrapper>
      <AO.FilterContainer>
        <AO.CustomInput
          type='button'
          value={selectedPartner?.name ? selectedPartner.name : 'Pilih Partner'}
      ...
      >
        <PartnerListModalContent
          handleChangePartner={(id, name) => {
            setGetOrdersParams({
              ...getOrdersParams,
              partner_id: id?.toString(),
              page: '1',
            });
            setSelectedPartner({ ...selectedPartner, id, name });
...
export const determineOrderStatus = (status: string): string => {
